...
class Attendee(Person):
    def __init__(self, id, age, ticket, total_drinks, total_foods,total_treatments, total_bathroom_visits, total_stage_visits, gender, activities):
        ...
        self.display_entered_at = datetime.datetime.now().time() 
    ...
    # main function for the attendee - runs in the threadpool
...
class SecurityStaff(threading.Thread):
    ...
    def run(self):
        while True:
            attendee = self.entrance.get_next_attendee()
            if attendee is None:
                break
            ...
            if attendee.ticket.type != "No ticket":
                ...
                attendee.display_entered_at = None
...
class Stage:
    def __init__(self, num_stages, artists_info):
        self.stages = [{'lock': threading.Lock(), 'genre': 'Pop'}, 
                       ...
                       {'lock': threading.Lock(), 'genre': 'Reggaeton'}]
...
class MenuItem:
    ...
    def __str__(self) -> str:
        # Return a formatted string showing name, price, and prep time
...
class Bathroom:
    def __init__(self, stalls_per_gender):
        ...
        self.persons_lock = {'Male': threading.Lock(), 'Female': threading.Lock()}
...
class FestivalSimulation:
    ...
    def start(self):        
        try: 
            ...
            bar_threads = [threading.Thread(target=bar.start) for bar in self.bars]
